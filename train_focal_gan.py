# train_focal_gan.py — 实验 B: Focal + Dice Reconstruction Loss
# =============================================================================
# 用 MultiTaskStressLoss (Focal + Dice) 替换 L1 重建损失。
# Focal 处理 1:6319 的奇异点极端失衡，Dice 直接优化重叠度指标。
#
# 判别器保持标准 DiscriminatorGNN（不改动 D 架构），
# 仅改进 Generator 的监督信号质量。
#
# 对比基线：train_gan_v5_compare.py (v5 Hinge GAN + L1)
#
# 支持 --resume <dir> 从中断的训练恢复
# =============================================================================

import torch, torch.nn as nn, torch.nn.functional as F, torch.optim as optim
from torch_geometric.loader import DataLoader
import numpy as np, os, sys, random, argparse, csv
...
def train_focal_gan(data_path, epochs=300, lr_g=2e-4, lr_d=5e-5, batch_size=8,
                    ...
                    device='cuda', save_root='./trained_model/focal_gan', seed=42,
                    resume_dir=None):
    set_seed(seed)
    ...
    if resume_dir is not None:
        ...
        ckpt_files = sorted([f for f in os.listdir(resume_dir) if f.startswith('ckpt_') and f.endswith('.pth')])
        if not ckpt_files:
            print(f"[Resume] No checkpoint found in {resume_dir}, starting fresh")
        else:
            ckpt_path = os.path.join(resume_dir, ckpt_files[-1])
            print(f"[Resume] Loading checkpoint: {ckpt_path}")
            ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
            start_epoch = ckpt['epoch']
            best_edge = ckpt.get('best_edge', 0.0)
            best_sing = ckpt.get('best_sing', 0.0)
            print(f"[Resume] Resuming from epoch {start_epoch} (best_sing={best_sing:.4f}, best_edge={best_edge:.4f})")

    if start_epoch >= epochs:
        print(f"[Resume] Already completed {start_epoch}/{epochs} epochs. Exiting.")
        return None, None, resume_dir, best_sing, best_edge

    # ── Setup save directory ──
    if resume_dir is not None:
        save_dir = resume_dir
        cur_time = os.path.basename(os.path.dirname(resume_dir))
        label = os.path.basename(resume_dir)
    else:
        label = 'focal_dice_gat_hinge'
        cur_time = datetime.now().strftime("%Y_%m_%d_%H%M")
        save_dir = os.path.join(save_root, cur_time, label)
    os.makedirs(save_dir, exist_ok=True)
    print(f"Experiment B: Focal + Dice Reconstruction Loss")
    ...
    print(f"G (GAT): {sum(p.numel() for p in G.parameters()):,} | "
          f"D (GAT): {sum(p.numel() for p in D.parameters()):,}")
    ...
    if resume_dir is not None and start_epoch > 0:
        ckpt_files = sorted([f for f in os.listdir(resume_dir) if f.startswith('ckpt_') and f.endswith('.pth')])
        if ckpt_files:
            ckpt = torch.load(os.path.join(resume_dir, ckpt_files[-1]), map_location=device, weights_only=False)
            G.load_state_dict(ckpt['G'])
            D.load_state_dict(ckpt['D'])
            if 'opt_g' in ckpt: opt_g.load_state_dict(ckpt['opt_g'])
            if 'opt_d' in ckpt: opt_d.load_state_dict(ckpt['opt_d'])
            if 'sg' in ckpt: sg.load_state_dict(ckpt['sg'])
            if 'sd' in ckpt: sd.load_state_dict(ckpt['sd'])
    ...
    if start_epoch == 0:
        ...
        with open(os.path.join(save_dir, 'hyperparams.txt'), 'w') as f:
            for k, v in dict(experiment='focal_dice_gan', epochs=epochs, lr_g=lr_g, lr_d=lr_d,
                             ...
                             loss='MultiTaskStressLoss(Focal+Dice)', w_sing=2.0, w_psl1=1.0, w_psl2=1.0,
                             sing_gamma=4.0, sing_alpha=0.95, psl_gamma=1.0, psl_alpha=0.75).items():
                f.write(f"{k}: {v}\n")
    else:
        # Restore best metrics from existing CSV
        if os.path.exists(log_path):
            with open(log_path) as f:
                reader = csv.DictReader(f)
                for row in reader:
                    sd_val = float(row['sing_dice'])
                    ed_val = float(row['edge_dice'])
                    if sd_val > best_sing:
                        best_sing = sd_val
                    if ed_val > best_edge:
                        best_edge = ed_val
            print(f"[Resume] Restored best metrics from CSV: best_sing={best_sing:.4f}, best_edge={best_edge:.4f}")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        pbar = tqdm(train_loader, desc=f"[FocalGAN] E{epoch+1}/{epochs}")
        for batch in pbar:
            ...
            opt_g.zero_grad()
            fn, fe = G(batch)
            g_recon, log_dict = criterion(fn, rn, fe, re)
            fn2, fe2 = add_disc_noise(fn, fe, d_noise)
            fs = D(batch, fn2, fe2)
            g_adv = -fs.mean()
        ...
        # Checkpoint every 50 epochs (with full state for resume)
        if (epoch + 1) % 50 == 0 or (epoch + 1) == epochs:
            torch.save({
                ...
                'best_sing': best_sing, 'best_edge': best_edge,
            }, os.path.join(save_dir, f'ckpt_{epoch+1}.pth'))

        print(f"[FocalGAN] E{epoch+1}: D={ls['d']/nb:.3f} G={ls['g']/nb:.3f} "
    ...
    print(f"\n[FocalGAN] Done! Best Sing Dice={best_sing:.4f} Edge Dice={best_edge:.4f}")
    return G, D, save_dir, best_sing, best_edge


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Experiment B: Focal + Dice Reconstruction Loss GAN')
    ...
    print("=" * 60)
    print("Experiment B: Focal + Dice Reconstruction Loss GAN")
    print("=" * 60)
    train_focal_gan(DATA, epochs=args.epochs, batch_size=args.batch_size,
                    device=DEV, seed=args.seed, lr_g=args.lr_g, lr_d=args.lr_d,
                    lambda_recon=args.lambda_recon, lambda_adv=args.lambda_adv,
